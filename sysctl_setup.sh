...
echo "# Improve responsiveness on high work load."
echo kern.sched.preempt_thresh=224 >> /etc/sysctl.conf
echo "# Disable terminal beep."
echo kern.vt.enable_bell=0 >> /etc/sysctl.conf
echo "# Enable users to mount drives."
echo vfs.usermount=1 >> /etc/sysctl.conf
echo "# Improve read/write permormance."
...
echo "# Disable creating *.core files in home directory."
echo kern.coredump=0 >> /etc/sysctl.conf
echo "# Speed up shutdown."
...
echo "# Fix stuttering issue on AMD CPUs."
